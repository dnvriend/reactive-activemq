...
import com.github.dnvriend.stream.{MessageBuilder, MessageExtractor, _}
...
object ActiveMqFlow {
  ...
  /**
    ...
    */
  def apply[S, T, M1, M2](source: Source[AckUTup[S], M1], sink: Sink[AckUTup[T], M2])
                         (implicit ec: ExecutionContext, system: ActorSystem): Flow[T, S, NotUsed] = {
    applyMat(source, sink)(Keep.none)
  }

  def applyMat[S, T, M1, M2, Mat](source: Source[AckUTup[S], M1], sink: Sink[AckUTup[T], M2])(combineMat: (M1, M2) ⇒ Mat)
                                 (implicit ec: ExecutionContext, system: ActorSystem): Flow[T, S, Mat] = {
  ...
  /**
    ...
    */
  def apply[S, T](consumerName: String, producerName: String, qos: Int = 8)
                 (implicit ec: ExecutionContext, system: ActorSystem, extractor: MessageExtractor[CamelMessage, S],
