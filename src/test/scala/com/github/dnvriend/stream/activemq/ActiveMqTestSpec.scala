...
trait ActiveMqTestSpec extends TestSpec {
  ...
  /**
    ...
    */
  def withAckBidiFlow(test: TestPublisher.Probe[AckUTup[Person]] ⇒ TestSubscriber.Probe[AckUTup[Person]] ⇒ Any)
                     (implicit backendFlow: Flow[Person, Person, NotUsed]): Unit = {
